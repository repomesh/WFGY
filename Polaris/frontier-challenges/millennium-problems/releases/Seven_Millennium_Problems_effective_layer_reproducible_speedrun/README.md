...
It lets reviewers download the ZIP, replay the public speedrun, inspect the strict theorem-debt matrices, verify the C03 original-theorem writeback result, and run the C04 two-way official-statement validation layer.
...
A Colab runner is provided for one-click-style reproduction.

| Notebook | Purpose |
|---|---|
| [WFGY_Seven_Millennium_Speedrun_Colab_Runner.ipynb](./WFGY_Seven_Millennium_Speedrun_Colab_Runner.ipynb) | Upload the speedrun ZIP, verify SHA256, run the public speedrun, run C03, run C04, measure timing, and export a results ZIP. |

Open in Colab:

[Open Colab Runner](https://colab.research.google.com/github/onestardao/WFGY/blob/main/Polaris/frontier-challenges/millennium-problems/releases/Seven_Millennium_Problems_effective_layer_reproducible_speedrun/WFGY_Seven_Millennium_Speedrun_Colab_Runner.ipynb)
...
verify SHA256
unzip the package
run the public speedrun runner
run the C03 original theorem writeback validator
run the C04 two-way official-statement validator
measure elapsed runtime
print terminal-style output
export a Colab results ZIP
```

Expected uploaded ZIP SHA256:

```txt
FA767D99BFCD55041FDBCC86C5BD62F781DF9B39C617E5B28E3BD461C3FFE042
```

Expected Colab final status:

```txt
final_colab_status: PASS
```

Expected result files generated by Colab:

```txt
COLAB_VALIDATION_REPORT.json
COLAB_TERMINAL_OUTPUT.txt
PUBLIC_RPG_RUN_SUMMARY.json
```
...
C04 official statement intake debt: 0
C04 two-way statement validation debt: 0
C04 two-way statement validation: PASS
...
→ C04 two-way official-statement validation
...
```txt
C03:
native/effective route → original theorem writeback validation

C04:
official statement payload → native coverage validation
native route → official statement return coverage validation
two-way statement validation data
```

This package publishes public validation data and validator scripts.
It does not publish the internal compiler engine.
...
10_OFFICIAL_STATEMENT_TWO_WAY_VALIDATION/
tools/
PROMPT.txt
...
* records C04 official statement intake and two-way validation debt
...
* checks C03 writeback validation
* checks C04 two-way official-statement validation data
...
C03 original theorem writeback validation layer.
...
C04 two-way official-statement validation data layer.

Main files:

```txt
10_OFFICIAL_STATEMENT_TWO_WAY_VALIDATION/README.md
10_OFFICIAL_STATEMENT_TWO_WAY_VALIDATION/ALL7_OFFICIAL_STATEMENT_PAYLOAD_INDEX.tsv
10_OFFICIAL_STATEMENT_TWO_WAY_VALIDATION/ALL7_OFFICIAL_TO_NATIVE_COVERAGE_MATRIX.tsv
10_OFFICIAL_STATEMENT_TWO_WAY_VALIDATION/ALL7_NATIVE_TO_OFFICIAL_COVERAGE_MATRIX.tsv
10_OFFICIAL_STATEMENT_TWO_WAY_VALIDATION/ALL7_TWO_WAY_STATEMENT_VALIDATION_REPORT.json
```

Purpose:

* publishes official-statement validation data
* checks official statement payload coverage
* checks official-to-native coverage
* checks native-to-official return coverage
* checks that the public package is not only a statement tag / receipt layer
* validates that the public statement payload, native route receipt, and official return layer are mutually aligned
...
```txt
This package publishes validation data and validator scripts.
It does not publish the internal compiler engine.
```
...
* requires the model to report the final speedrun status from the included scoreboard, ledger, C03, and C04 files
* prints both C03 and C04 final status lines
...
* reports C04 two-way official-statement validation status
...
all7_original_theorem_writeback_debt: 0
two_way_statement_validation_status: OFFICIAL_STATEMENT_TWO_WAY_VALIDATION_ALL7_PASS
all7_official_statement_intake_debt: 0
all7_two_way_statement_validation_debt: 0
```
...
### C04 two-way official-statement validator

```txt
tools/validate_two_way_statement_validation.py
```

Purpose:

* reads the C04 two-way validation reports
* checks all seven problem lanes
* verifies official statement intake debt is 0
* verifies two-way statement validation debt is 0
* verifies official-to-native and native-to-official validation data
* reports the final C04 validation status
...
python tools/validate_two_way_statement_validation.py
```
...
```txt
SHA256 check: PASS
Public RPG Speedrun Runner: PASS
C03 Original Theorem Writeback Validator: PASS
C04 Two-Way Official Statement Validation Data Validator: PASS
Final Colab Status: PASS
```

Expected terminal-style output includes:

```txt
7/7 bosses processed.
Total open strict debt cells: 0
C03 writeback status: ORIGINAL_THEOREM_WRITEBACK_VALIDATED_ALL7_PASS
Two-way statement validation status: OFFICIAL_STATEMENT_TWO_WAY_VALIDATION_ALL7_PASS
Official statement intake debt: 0
Two-way statement validation debt: 0
```

Expected Colab output ZIP includes:
...
7. 10_OFFICIAL_STATEMENT_TWO_WAY_VALIDATION/ALL7_TWO_WAY_STATEMENT_VALIDATION_REPORT.json
8. tools/run_public_rpg_speedrun.py
9. tools/validate_c03_writeback.py
10. tools/validate_two_way_statement_validation.py
11. PROMPT.txt
12. WFGY_Seven_Millennium_Speedrun_Colab_Runner.ipynb
```
...
README.md
Seven_Millennium_Problems_effective_layer_reproducible_speedrun.zip
WFGY_Seven_Millennium_Speedrun_Colab_Runner.ipynb
```
